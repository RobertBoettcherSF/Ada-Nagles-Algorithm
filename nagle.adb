      ...
      if New_Data'Length > 0 then
         ...
         if Window_Size >= MSS and then Has_Enough_Data(Buffer, MSS) then
            ...
            if Unacked = Has_Unacked then
               Buffer.Append((Data => New_Data, Size => New_Data'Length));
               Send_Now := False;
            else
               -- No unacked data: send immediately
               Send_Now := True;
               Packet_To_Send := (Data => New_Data, Size => New_Data'Length);
            end if;
         end if;
      else
         -- No new data: do not send
         Send_Now := False;
         Packet_To_Send := (Data => (1 .. 0 => 0), Size => 0);
      end if;
   exception
      when others =>
